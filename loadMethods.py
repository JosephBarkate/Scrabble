...
def getWordScore(word, n):
    ...
    """
    wordScore = 0
    #loop over the word and find the corresponding values
    for i in word:
        wordScore += SCRABBLE_LETTER_VALUES[i]
    #now multiply the total letters by length
    wordScore *= len(word)
    #if the length is equal to the total letters n
    if len(word) == n:
        wordScore += 50
    return wordScore
...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    #copy the hand as to not mutate
    newHand = dict(hand)
    for i in word:
        #if there is more than one letter
        if newHand.get(i,0) > 1:
            newHand[i] -= 1
        #if there is only 1 letter delete
        elif newHand.get(i,0) == 1:
            del newHand[i]
    return newHand
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        #print("Current Hand: " + ' '.join(list(hand.keys())))
       ...
       # If the word is not valid:
        ...
        # Otherwise (the word is valid):
            ...
            wordPoints = getWordScore(newWord, n)
            totalPoints += wordPoints
            print("\"" + newWord +"\"" + " earned " + str(wordPoints) + " points. Total: " + str(totalPoints) + " points")
            # Update the hand 
